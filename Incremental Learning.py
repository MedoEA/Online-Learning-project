...
if __name__ == '__main__':
    ...
    IL = (best_action == 1).cumsum(axis=1) / np.arange(1, len(selected_action)+1)
    plt.plot(IL.mean(axis=0), label='Incremental Learning', ls='--')
    #plt.fill_between(np.arange(1,1401), IL.mean(axis=0) - IL.std(axis=0),IL.mean(axis=0) + IL.std(axis=0), alpha=0.3 );
