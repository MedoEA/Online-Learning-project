...
if __name__ == '__main__':
    ...
    D_O_C = (best_action == 1).cumsum(axis=1) / np.arange(1, len(selected_action)+1)
    plt.plot(D_O_C.mean(axis=0), label='Discard old C values', ls='--')
    #plt.fill_between(np.arange(1,1401), D_O_C.mean(axis=0) - D_O_C.std(axis=0),D_O_C.mean(axis=0) + D_O_C.std(axis=0), alpha=0.3 );
