...
if __name__ == '__main__':
    ...
    for iteration in range(10):
        ...
        # discarding points
        exclude = ex_cons.pop(9)
        avg_resp_time.pop(exclude)
    ...
    K_I_C = (best_action == 1).cumsum(axis=1) / np.arange(1, len(selected_action)+1)
    plt.plot(K_I_C.mean(axis=0), label='Keep initail C values', ls='--')
    #plt.fill_between(np.arange(1,2001), K_I_C.mean(axis=0) - K_I_C.std(axis=0),K_I_C.mean(axis=0) + K_I_C.std(axis=0), alpha=0.3 );
